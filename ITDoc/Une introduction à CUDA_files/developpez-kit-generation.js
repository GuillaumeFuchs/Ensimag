...
/******************************************* VARIABLES GLOBALES *******************************************************************/
/**********************************************************************************************************************************/
var hauteurSommaire, largeurSommaire, hauteurDroiteSommaire, margeBas, classNavSom, classFixeMarge, classInfoAuth, classToggle, TogglePlier, ToggleDeplier, hauteurContenuArticle, verifPresenceFixeMarge, verifPresenceInfoAuth, articleBody, barreNavigation;
var cheminImageKit = "http://www.developpez.com/template/kit/";
...
jQuery(function($) {
	...
	margeBas               = 60 + jQuery("#gabarit_pied").height();
...
function CacherMontreSommaireFAQ(elementId, IdImage1){
	...
	if (ObjetCode.css("display") == 'none') {
		...
		ObjetIdImage1.attr('src', cheminImageKit + 'kitmoins.gif');
	} else {
		ObjetCode.css("display", "none");	// Afficher le block
		ObjetIdImage1.attr('src', cheminImageKit + 'kitplus.gif');
	}
}
...
function popup_modal_auteur() {
	...
	jQuery(".poplight").click( function() {
		...
		DivPopupAuteurs.fadeIn().css({
			'width': LargeurPopup
		}).prepend('<a href="#" class="close"><img src="http://www.developpez.com/template/kit/fermeture-icone.png" class="CloturePopup" title="Fermer Pop-pup" alt="fermeture" /></a>');
