...
Le modèles de Black et Scholes permet de modéliser le cours des actions. Mais il permet aussi de modéliser l'évolution des indices. Il repose sur les hypothèses suivantes : \\
\begin{itemize}
...
\item[•] Le taux sans risque r est constant quelle que soit la maturité\\
\end{itemize}

\noindent Dans le modèle de Black-Scholes, le cours du sous-jacent S vérifie l'équation stochastique : \\
...
\subsection{Modèle des taux}
Point sur les modèles (Vasicek, Cox Ingersoll Ross, Black Derman Toy, Heath Jarrow Morton, Hull et White)
Mais on choisit pour le moment un modèle simple de taux constants. \\
...
On constate également la présence d'un lien entre l'interface et la base de données en effet, lorsque l'utilisateur séléectionne de nouvelle date pour voir l'évolution des différents indices, il n'est pas nécessaire de faire appel au moteur de calcul.
