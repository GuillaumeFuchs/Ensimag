...
__device__ float payoff_barrier(
	int N,
	int tid,
	int size,
	float K,
	...
	float* per_block_results_price)
{
		...
		//Calcul du payoff
		sdata_price[threadIdx.x] = payoff_barrier(N, tid, size, K, d_coeff, d_bu, d_bl, d_path);
