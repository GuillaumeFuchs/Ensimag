	...
	float* d_path) 
{
	...
	mc_barrier_u<<<dimGrid, dimBlock, 2*(dimBlock.x)*sizeof(float)>>>(N, size_, nUseThreads, (float)Strike_, d_coeff, d_bu, d_path, d_per_block_results_price, d_per_block_results_ic);
